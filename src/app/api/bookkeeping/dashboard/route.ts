...
export async function GET() {
    try {
        ...
        // ---------- TULOT / MENOT / TULOS (12 kk) ----------
        const finance12 = await prisma.bookkeepingEntry.findMany({
            where: {
                date: {
 ...
 const key = `${String(date.getMonth() + 1).padStart(2, "0")}/${String(date.getFullYear()).slice(2)}`;
            ...
            const monthEntries = finance12.filter(
                (e) =>
